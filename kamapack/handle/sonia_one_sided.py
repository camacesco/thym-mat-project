...
class SoniaOneSided(Sonia):

    def __init__(self, data_seqs = [], gen_seqs = [], chain_type = 'humanTRB',
                 load_dir = None, feature_file = None, data_seq_file = None, gen_seq_file = None, 
                 log_file = None, load_seqs = True,max_depth = 25, max_L = 30, include_indep_genes = False, 
                 include_joint_genes = True, min_energy_clip = -5, max_energy_clip = 10, seed = None,
                 custom_pgen_model=None,l2_reg=0.,l1_reg=0.,vj=False, 
                 feat_side='left', # <= HERE
                ):
        Sonia.__init__(self, data_seqs=data_seqs, gen_seqs=gen_seqs, chain_type=chain_type, 
                       ...
                       seed = seed,l2_reg=l2_reg,l1_reg=l1_reg,vj=vj)
        ...
        if feat_side in ['left', 'right'] : self.feat_side=feat_side
        else : raise IOError( 'Available options for `feat_side` are `left` or `right`.' ) 
        ...
        if any([x is not None for x in [load_dir, feature_file]]):
            ...
            self.add_features(include_indep_genes = include_indep_genes, include_joint_genes = include_joint_genes, 
                              custom_pgen_model = custom_pgen_model)
    ...
    def set_gauge(self):
        ...
        if self.feat_side == 'left' :
            ...
            delta_Z=np.sum([np.log(g) for g in Gs_plus])
